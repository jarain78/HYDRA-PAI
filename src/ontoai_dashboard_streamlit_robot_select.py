...
I18N = {
    "en": {
        ...
        "robot": "Robot",
        "robot_panda": "Panda (Franka)",
        "robot_mycobot": "MyCobot 280 M5",
        "mycobot_urdf": "MyCobot URDF path",
    ...
    "es": {
        ...
        "robot_panda": "Panda (Franka)",
        "robot_mycobot": "MyCobot 280 M5",
        "mycobot_urdf": "Ruta URDF MyCobot",
...
ROBOT_MODEL_DEFAULT = os.getenv("HYDRA_ROBOT", "panda").strip().lower()

# MyCobot URDF (relative to repo root by default)
MYCOBOT_URDF_DEFAULT = rel_or_abs(
    os.getenv("HYDRA_MYCOBOT_URDF", "RobotModels/mycobot_280_m5/mycobot_280_m5.urdf"),
    REPO_ROOT,
)

# MyCobot auto-placement near the first object (heuristic)
MYCOBOT_AUTO_PLACE_DEFAULT = True
MYCOBOT_OFFSET_X_DEFAULT = -0.20  # meters (relative to first object's x)
MYCOBOT_OFFSET_Y_DEFAULT = -0.35  # meters (relative to first object's y)

# Generic robot placement defaults
ROBOT_BASE_POS_DEFAULT = [0.0, -0.35, 0.0]
...
def panda_open_gripper(robot_id: int) -> None:
    for fj in PANDA_FINGER_JOINTS:
        p.setJointMotorControl2(robot_id, fj, p.POSITION_CONTROL, targetPosition=0.04, force=50)


def panda_close_gripper(robot_id: int) -> None:
    for fj in PANDA_FINGER_JOINTS:
        p.setJointMotorControl2(robot_id, fj, p.POSITION_CONTROL, targetPosition=0.0, force=80)


def panda_home(robot_id: int) -> None:
    home = [0.0, -0.6, 0.0, -2.2, 0.0, 1.6, 0.8]
    for j, q in zip(PANDA_ARM_JOINTS, home):
        p.resetJointState(robot_id, j, q)
        p.setJointMotorControl2(robot_id, j, p.POSITION_CONTROL, targetPosition=q, force=200)
    panda_open_gripper(robot_id)
...
# ============================================================
# MyCobot 280 (best-effort home for visualization)
# ============================================================

def mycobot_home(robot_id: int) -> None:
    """Set a reasonable home pose for MyCobot in PyBullet.

    This is *only* for visualization / consistent starting posture.
    The project execution (pick&place) is implemented for Panda by default.
    """
    # Try to find up to 6 revolute joints and set them to 0 rad.
    joint_indices = []
    ...
    for j in joint_indices:
        ...
        p.setJointMotorControl2(robot_id, j, p.POSITION_CONTROL, targetPosition=0.0, force=80)

    for _ in range(int(0.4 * SIM_HZ)):
        p.stepSimulation()
        time.sleep(DT)
def panda_move_ee(robot_id: int, target_pos, target_orn, duration: float, label: str = "", max_joint_vel: float = MAX_JOINT_VEL, force: float = DEFAULT_FORCE) -> None:
    steps = max(1, int(duration * SIM_HZ))
    max_step = max_joint_vel * DT
    q_ik = p.calculateInverseKinematics(robot_id, PANDA_EE_LINK, targetPosition=target_pos, targetOrientation=target_orn, maxNumIterations=200, residualThreshold=1e-4)
    q_goal = list(q_ik[:7])
    q = [p.getJointState(robot_id, j)[0] for j in PANDA_ARM_JOINTS]
    ...
    for i in range(steps):
        ...
        for j, qj in zip(PANDA_ARM_JOINTS, q):
            try:
...
def grasp_with_constraint(robot_id: int, obj_id: int) -> int:
    ee_state = p.getLinkState(robot_id, PANDA_EE_LINK)
...
def execute_pick_and_place(robot_id: int, obj_id: int, dst_room: str) -> List[str]:
    logs = []
    ...
    panda_open_gripper(robot_id)
    panda_move_ee(robot_id, [ox, oy, 0.28], down_orn, duration=1.5)
    pre_z = max(0.03, 0.08 - GRASP_Z_OFFSET)
    panda_move_ee(robot_id, [ox, oy, pre_z], down_orn, duration=1.2)

    panda_close_gripper(robot_id)
    for _ in range(int(0.35 * SIM_HZ)):
        p.stepSimulation()
        time.sleep(DT)

    cid = grasp_with_constraint(robot_id, obj_id)
    logs.append(f"GRASP constraint id={cid}")

    panda_move_ee(robot_id, [ox, oy, 0.30], down_orn, duration=1.4)
    panda_move_ee(robot_id, [dx, dy, 0.30], down_orn, duration=2.6)

    place_z = max(0.05, 0.10 - GRASP_Z_OFFSET)
    panda_move_ee(robot_id, [dx, dy, place_z], down_orn, duration=1.2)
    ...
    logs.append("RELEASE")
    panda_move_ee(robot_id, [dx, dy, 0.30], down_orn, duration=1.2)
    logs.append("DONE")
    return logs
...
def setup_pybullet_world(
    ...
    *,
    ...
    mycobot_urdf: Optional[str] = None,
    robot_base_pos: Optional[List[float]] = None,
    use_fixed_base: bool = True,
    mycobot_auto_place: bool = True,
    mycobot_offset_xy: Tuple[float, float] = (MYCOBOT_OFFSET_X_DEFAULT, MYCOBOT_OFFSET_Y_DEFAULT),
) -> Tuple[int, Dict[str, int], str]:
    """Create a fresh PyBullet world and load the selected robot.

    Returns:
        robot_id, sim_objects, robot_model_used
    """
    if p.isConnected():
        try:
            p.disconnect()
    ...
    robot_model = (robot_model or "panda").strip().lower()
    robot_base_pos = robot_base_pos or list(ROBOT_BASE_POS_DEFAULT)

    # --- Load robot
    # Auto-place MyCobot close enough to reach the first object.
    # Heuristic: place the base at (obj_x + offset_x, obj_y + offset_y, 0).
    if robot_model in {"mycobot", "mycobot280", "mycobot_280", "mycobot280-m5", "mycobot_280_m5"}:
        if mycobot_auto_place and (robot_base_pos is None) and ycb_positions:
            ox, oy, _oz = ycb_positions[0]
            offx, offy = mycobot_offset_xy
            robot_base_pos = [float(ox + offx), float(oy + offy), 0.0]
        # If still None, fall back to default base.
        robot_base_pos = robot_base_pos or list(ROBOT_BASE_POS_DEFAULT)
    
# --- Load robot
    if robot_model in {"panda", "franka", "franka_panda"}:
        robot_urdf = "franka_panda/panda.urdf"
        robot_id = p.loadURDF(robot_urdf, basePosition=robot_base_pos, useFixedBase=use_fixed_base)
        panda_home(robot_id)
        robot_model_used = "panda"

    elif robot_model in {"mycobot", "mycobot280", "mycobot_280", "mycobot280-m5", "mycobot_280_m5"}:
        # Resolve URDF path for MyCobot
        mycobot_urdf = mycobot_urdf or MYCOBOT_URDF_DEFAULT
        if mycobot_urdf.startswith(("file://", "http://", "https://")):
            # For now we only support local paths for URDFs
            raise RuntimeError("MyCobot URDF must be a local filesystem path (no URL scheme).")
        urdf_path = str(Path(mycobot_urdf).expanduser().resolve())
        if not os.path.isfile(urdf_path):
            raise FileNotFoundError(f"MyCobot URDF not found: {urdf_path}")
        robot_id = p.loadURDF(urdf_path, basePosition=robot_base_pos, useFixedBase=use_fixed_base)
        # Set a reasonable home pose (best-effort: first 6 revolute joints)
        try:
            mycobot_home(robot_id)
        except Exception:
            pass
        robot_model_used = "mycobot"

    else:
        raise ValueError(f"Unknown robot_model='{robot_model}'. Supported: panda, mycobot")
    ...
    if gui:
        p.resetDebugVisualizerCamera(
            ...
            cameraYaw=35,
            cameraPitch=-35,
            cameraTargetPosition=[0.35, 0.0, 0.15],
        )
    return robot_id, sim_objects, robot_model_used
    ...
    if apply_execution:
        ...
        if move_tasks:
            ...
            if bid is not None and dst in set(ROOM_NAMES):
                if st.session_state.get("robot_model_used", "panda") != "panda":
                    exec_logs.append(f"Execution skipped: robot_model='{st.session_state.get('robot_model_used')}' (executor implemented for Panda).")
                else:
                    exec_logs = execute_pick_and_place(robot_id, bid, dst)
...
def init_state(gui: bool, ycb_repo_dir: str, ycb_objects: List[str], ycb_positions: List[Tuple[float, float, float]],
               ...
               robot_model: str, mycobot_urdf: str, use_fixed_base: bool = True, mycobot_auto_place: bool = True, mycobot_offset_xy: Tuple[float, float] = (MYCOBOT_OFFSET_X_DEFAULT, MYCOBOT_OFFSET_Y_DEFAULT)) -> None:
    ...
    robot_id, sim_objects, robot_model_used = setup_pybullet_world(
        gui, ycb_repo_dir, ycb_objects, ycb_positions,
        robot_model=robot_model,
        mycobot_urdf=mycobot_urdf,
        use_fixed_base=use_fixed_base,
        mycobot_auto_place=mycobot_auto_place,
        mycobot_offset_xy=mycobot_offset_xy,
    )
    st.session_state["robot_model_used"] = robot_model_used
...
def main():
    ...
    with st.sidebar:
        ...
        # Robot selection
        robot_label = st.selectbox(
            tr("robot"),
            [tr("robot_panda"), tr("robot_mycobot")],
            index=0 if st.session_state.get("robot_model", ROBOT_MODEL_DEFAULT) in {"panda","franka","franka_panda"} else 1,
        )
        robot_model = "panda" if robot_label == tr("robot_panda") else "mycobot"
        st.session_state["robot_model"] = robot_model

        # MyCobot URDF path (only used if mycobot is selected)
        mycobot_urdf = MYCOBOT_URDF_DEFAULT
        if robot_model == "mycobot":
            mycobot_urdf = st.text_input(tr("mycobot_urdf"), value=mycobot_urdf)
        st.session_state["mycobot_urdf"] = mycobot_urdf

        # MyCobot placement controls
        mycobot_auto_place = True
        mycobot_offx = MYCOBOT_OFFSET_X_DEFAULT
        mycobot_offy = MYCOBOT_OFFSET_Y_DEFAULT
        if robot_model == "mycobot":
            mycobot_auto_place = st.toggle("Auto-place MyCobot near first object", value=MYCOBOT_AUTO_PLACE_DEFAULT)
            cox, coy = st.columns(2)
            with cox:
                mycobot_offx = st.slider("MyCobot offset X (m)", -0.80, 0.80, float(MYCOBOT_OFFSET_X_DEFAULT), 0.05)
            with coy:
                mycobot_offy = st.slider("MyCobot offset Y (m)", -0.80, 0.80, float(MYCOBOT_OFFSET_Y_DEFAULT), 0.05)
        st.session_state["mycobot_auto_place"] = bool(mycobot_auto_place)
        st.session_state["mycobot_offset_xy"] = (float(mycobot_offx), float(mycobot_offy))

        use_fixed_base = st.toggle(tr("use_fixed_base"), value=ROBOT_USE_FIXED_BASE_DEFAULT)
        st.session_state["use_fixed_base"] = use_fixed_base

        # If robot changed, force re-init
        prev_robot = st.session_state.get("_prev_robot_model", None)
        if prev_robot is None:
            st.session_state["_prev_robot_model"] = robot_model
        elif prev_robot != robot_model:
            st.session_state["_prev_robot_model"] = robot_model
    ...
    init_state(True, ycb_repo_dir, ycb_objects, ycb_positions, resolved_onto, iri_object, iri_room, yolo_weights,
              st.session_state.get('robot_model', ROBOT_MODEL_DEFAULT),
              st.session_state.get('mycobot_urdf', MYCOBOT_URDF_DEFAULT),
              st.session_state.get('use_fixed_base', ROBOT_USE_FIXED_BASE_DEFAULT),
              st.session_state.get('mycobot_auto_place', MYCOBOT_AUTO_PLACE_DEFAULT),
              st.session_state.get('mycobot_offset_xy', (MYCOBOT_OFFSET_X_DEFAULT, MYCOBOT_OFFSET_Y_DEFAULT)))
