...
AUTO_ROBOT_BASE_DEFAULT = True
ROBOT_BASE_POS_DEFAULT = (0.25, -0.22, 0.0)
...
def compute_effective_robot_base(ycb_positions: List[Tuple[float, float, float]], auto_robot_base: bool, robot_base_pos: Tuple[float, float, float], robot_base_rpy_deg: Tuple[float, float, float]) -> Tuple[List[float], List[float]]:
    if auto_robot_base and ycb_positions:
        ox, oy, _oz = ycb_positions[0]
        base_pos = [float(ox - 0.18), float(oy - 0.22), 0.0]
    else:
        base_pos = [float(robot_base_pos[0]), float(robot_base_pos[1]), float(robot_base_pos[2])]
    base_rpy_deg = [float(robot_base_rpy_deg[0]), float(robot_base_rpy_deg[1]), float(robot_base_rpy_deg[2])]
    return base_pos, base_rpy_deg

def apply_dynamic_robot_base_pose(robot_id: int, base_pos: List[float], base_rpy_deg: List[float], *, use_fixed_base: bool) -> Tuple[List[float], List[float]]:
    base_quat = p.getQuaternionFromEuler([math.radians(v) for v in base_rpy_deg])
    p.resetBasePositionAndOrientation(robot_id, base_pos, base_quat)
    if use_fixed_base:
        place_robot_on_floor(robot_id, extra_z=0.003)
    new_pos, new_quat = p.getBasePositionAndOrientation(robot_id)
    new_rpy_deg = [math.degrees(v) for v in p.getEulerFromQuaternion(new_quat)]
    return list(new_pos), list(new_rpy_deg)

def make_world_signature(*, gui: bool, ycb_repo_dir: str, ycb_objects: List[str], ycb_positions: List[Tuple[float, float, float]], onto_uri: str, iri_object: str, iri_room: str, yolo_weights: str, robot_urdf: str, use_fixed_base: bool, auto_robot_base: bool, robot_base_pos: Tuple[float, float, float], robot_base_rpy_deg: Tuple[float, float, float]) -> str:
    payload = {
        "gui": bool(gui),
        "ycb_repo_dir": str(ycb_repo_dir),
        "ycb_objects": list(ycb_objects),
        "ycb_positions": [list(v) for v in ycb_positions],
        "onto_uri": str(onto_uri),
        "iri_object": str(iri_object),
        "iri_room": str(iri_room),
        "yolo_weights": str(yolo_weights),
        "robot_urdf": str(robot_urdf),
        "use_fixed_base": bool(use_fixed_base),
        "auto_robot_base": bool(auto_robot_base),
        "robot_base_pos": list(robot_base_pos),
        "robot_base_rpy_deg": list(robot_base_rpy_deg),
    }
...
def setup_pybullet_world(gui: bool, ycb_repo_dir: str, ycb_objects: List[str], ycb_positions: List[Tuple[float, float, float]], *, robot_urdf: str, use_fixed_base: bool = True, auto_robot_base: bool = True, robot_base_pos: Tuple[float, float, float] = ROBOT_BASE_POS_DEFAULT, robot_base_rpy_deg: Tuple[float, float, float] = ROBOT_BASE_RPY_DEG_DEFAULT):
    if p.isConnected():
        try:
            p.disconnect()
        except Exception:
    ...
    effective_robot_base_pos, effective_robot_base_rpy_deg = compute_effective_robot_base(
    ...
    log(f"Auto robot base: {auto_robot_base}")
    log(f"Requested robot base position: {list(robot_base_pos)}")
    log(f"Requested robot base RPY deg: {list(robot_base_rpy_deg)}")
    log(f"Effective robot base position: {effective_robot_base_pos}")
    log(f"Effective robot base RPY deg: {effective_robot_base_rpy_deg}")

    try:
        robot_id = p.loadURDF(
            fixed_urdf,
            basePosition=list(effective_robot_base_pos),
            baseOrientation=p.getQuaternionFromEuler([math.radians(v) for v in effective_robot_base_rpy_deg]),
...
# Streamlit init + main
# ============================================================
def init_state(gui: bool, ycb_repo_dir: str, ycb_objects: List[str], ycb_positions: List[Tuple[float, float, float]], onto_uri: str, iri_object: str, iri_room: str, yolo_weights: str, robot_urdf: str, use_fixed_base: bool = True, auto_robot_base: bool = True, robot_base_pos: Tuple[float, float, float] = ROBOT_BASE_POS_DEFAULT, robot_base_rpy_deg: Tuple[float, float, float] = ROBOT_BASE_RPY_DEG_DEFAULT) -> None:
    world_signature = make_world_signature(
        gui=gui, ycb_repo_dir=ycb_repo_dir, ycb_objects=ycb_objects, ycb_positions=ycb_positions, onto_uri=onto_uri, iri_object=iri_object, iri_room=iri_room, yolo_weights=yolo_weights, robot_urdf=robot_urdf, use_fixed_base=use_fixed_base, auto_robot_base=auto_robot_base, robot_base_pos=robot_base_pos, robot_base_rpy_deg=robot_base_rpy_deg
    )
    if st.session_state.get("initialized", False):
        if st.session_state.get("world_signature") != world_signature:
            log("World configuration changed. Rebuilding simulation state...")
            for key in ["initialized", "robot_id", "sim_objects", "arm_joints", "ee_link", "onto_info", "hierarchy", "yolo", "clips_env", "cam", "last_result"]:
                st.session_state.pop(key, None)
        else:
            return
    st.session_state["ui_logs"] = []
    log("Initializing world...")
    robot_id, sim_objects, arm_joints, ee_link = setup_pybullet_world(gui, ycb_repo_dir, ycb_objects, ycb_positions, robot_urdf=robot_urdf, use_fixed_base=use_fixed_base, auto_robot_base=auto_robot_base, robot_base_pos=robot_base_pos, robot_base_rpy_deg=robot_base_rpy_deg)
    ...
    effective_base_pos, effective_base_rpy_deg = compute_effective_robot_base(ycb_positions, auto_robot_base, robot_base_pos, robot_base_rpy_deg)
    st.session_state.update({"robot_id": robot_id, "sim_objects": sim_objects, "arm_joints": arm_joints, "ee_link": ee_link, "onto_info": onto_info, "hierarchy": hierarchy, "yolo": yolo, "clips_env": clips_env, "cam": CameraConfig(), "initialized": True, "running": False, "last_result": None, "world_signature": world_signature, "auto_robot_base": auto_robot_base, "robot_base_pos": tuple(robot_base_pos), "robot_base_rpy_deg": tuple(robot_base_rpy_deg), "effective_robot_base_pos": effective_base_pos, "effective_robot_base_rpy_deg": effective_base_rpy_deg})

def update_robot_base_pose_if_needed(*, ycb_positions: List[Tuple[float, float, float]], auto_robot_base: bool, robot_base_pos: Tuple[float, float, float], robot_base_rpy_deg: Tuple[float, float, float], use_fixed_base: bool) -> None:
    ...
    if robot_id is None:
        return
    prev = (
        bool(st.session_state.get("auto_robot_base", True)),
        tuple(st.session_state.get("robot_base_pos", ROBOT_BASE_POS_DEFAULT)),
        tuple(st.session_state.get("robot_base_rpy_deg", ROBOT_BASE_RPY_DEG_DEFAULT)),
    )
    now = (bool(auto_robot_base), tuple(robot_base_pos), tuple(robot_base_rpy_deg))
    if prev == now:
        return
    effective_pos, effective_rpy_deg = compute_effective_robot_base(ycb_positions, auto_robot_base, robot_base_pos, robot_base_rpy_deg)
    applied_pos, applied_rpy_deg = apply_dynamic_robot_base_pose(robot_id, effective_pos, effective_rpy_deg, use_fixed_base=use_fixed_base)
    st.session_state["auto_robot_base"] = auto_robot_base
    st.session_state["robot_base_pos"] = tuple(robot_base_pos)
    st.session_state["robot_base_rpy_deg"] = tuple(robot_base_rpy_deg)
    st.session_state["effective_robot_base_pos"] = applied_pos
    st.session_state["effective_robot_base_rpy_deg"] = applied_rpy_deg
    log(f"Dynamic robot base update applied: pos={applied_pos}, rpy_deg={applied_rpy_deg}")
...
def main():
    ...
    with st.sidebar:
        ...
        auto_robot_base = st.toggle("Auto place robot base near object", value=st.session_state.get("auto_robot_base", AUTO_ROBOT_BASE_DEFAULT))
        robot_base_pos_str = st.text_input("Robot base position (x, y, z)", value=", ".join(map(str, st.session_state.get("robot_base_pos", ROBOT_BASE_POS_DEFAULT))), disabled=auto_robot_base)
        robot_base_rpy_deg_str = st.text_input("Robot base orientation RPY (deg)", value=", ".join(map(str, st.session_state.get("robot_base_rpy_deg", ROBOT_BASE_RPY_DEG_DEFAULT))))
    ...
    init_state(True, ycb_repo_dir, ycb_objects, ycb_positions, resolved_onto, iri_object, iri_room, yolo_weights, robot_urdf, use_fixed_base, auto_robot_base, robot_base_pos, robot_base_rpy_deg)
    update_robot_base_pose_if_needed(ycb_positions=ycb_positions, auto_robot_base=auto_robot_base, robot_base_pos=robot_base_pos, robot_base_rpy_deg=robot_base_rpy_deg, use_fixed_base=use_fixed_base)
    try:
    ...
    with left:
        ...
        st.caption(f"Effective robot base pose: pos={st.session_state.get('effective_robot_base_pos', [])} | rpy_deg={st.session_state.get('effective_robot_base_rpy_deg', [])}")
