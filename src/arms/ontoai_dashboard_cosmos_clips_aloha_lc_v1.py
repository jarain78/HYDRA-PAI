...
ALOHA_URDF_DEFAULT = str(Path(os.getenv("HYDRA_ALOHA_URDF", "src/RobotModels/ALOHA_MED/robot.urdf")).expanduser().resolve())
...
def get_aloha_joint_map(robot_id: int) -> Dict[str, int]:
    return find_joint_indices_by_name(robot_id, ALOHA_ARM_JOINT_NAMES)
...
def setup_pybullet_world(
    ...
    *,
    ...
    missing = [jn for jn in ALOHA_ARM_JOINT_NAMES if jn not in jmap]# + ALOHA_GRIPPER_JOINT_NAMES if jn not in jmap]
    if missing:
