...
# -*- coding: utf-8 -*-
"""
ontoai_gui_ycb_cosmos_clips_aloha.py

Adapted version of ontoai_gui_ycb_cosmos_clips.py to work ONLY with the
ALOHA LC v1 robot URDF.

Main changes:
- Removed Panda / MyCobot selection.
- Added robust ALOHA LC v1 URDF loading with local asset resolution.
- Added ALOHA LC v1 joint discovery, home pose, IK motion and simple gripper control.
- Execution is now performed only with ALOHA LC v1.

Run:
  pip install streamlit pybullet owlready2 clipspy ultralytics transformers torch imageio imageio-ffmpeg
  streamlit run ontoai_gui_ycb_cosmos_clips_aloha.py
"""
...
I18N = {
    "en": {
        "app_title": "OntoAI GUI - ALOHA LC v1",
        "headline": "Cosmos (high-level) → CLIPS (plan) → PyBullet ALOHA LC v1 (execution)",
        "subtitle": "Cosmos uses the PyBullet camera + ontology to decide intents; CLIPS plans; the ALOHA LC v1 robot executes.",
        "controls": "Controls",
        "language": "Language",
        "mode": "Mode",
        ...
        "urdf_path": "ALOHA LC v1 URDF path",
        "use_fixed_base": "Use fixed base",
    },
    "es": {
        "app_title": "OntoAI GUI - ALOHA LC v1",
        "headline": "Cosmos (alto nivel) → CLIPS (plan) → PyBullet ALOHA LC v1 (ejecución)",
        "subtitle": "Cosmos usa la cámara de PyBullet + ontología para decidir intents; CLIPS planifica; el robot ALOHA LC v1 ejecuta.",
        "controls": "Controles",
        "language": "Idioma",
        "mode": "Modo",
        ...
        "urdf_path": "Ruta URDF ALOHA LC v1",
...
ALOHA_URDF_DEFAULT = str(Path(os.getenv("HYDRA_ALOHA_URDF", "src/RobotModels/ALOHA_LC_v1/aloha_lc_v1.urdf")).expanduser().resolve())
...
ALOHA_ARM_JOINT_NAMES = [
    "joint_1_shoulder_pan",
    "joint_2_shoulder_lift",
    "joint_3_elbow",
    "joint_4_wrist_1",
    "joint_5_wrist_2",
    "joint_6_wrist_3",
]
ALOHA_GRIPPER_JOINT_NAMES = ["gripper_left_slide", "gripper_right_slide"]
ALOHA_GRIPPER_JOINT_NAME = "joint_6_wrist_3"
...
def get_aloha_joint_map(robot_id: int) -> Dict[str, int]:
    return find_joint_indices_by_name(robot_id, ALOHA_ARM_JOINT_NAMES + ALOHA_GRIPPER_JOINT_NAMES)

def aloha_home(robot_id: int, jmap: Dict[str, int]) -> None:
    home = {
        "joint_1_shoulder_pan": 0.0,
        "joint_2_shoulder_lift": 0.9,
        "joint_3_elbow": 1.4,
        "joint_4_wrist_1": 0.6,
        "joint_5_wrist_2": 0.0,
        "joint_6_wrist_3": 0.0,
        "gripper_left_slide": 0.017,
        "gripper_right_slide": 0.017,
    }
...
def aloha_open_gripper(robot_id: int, jmap: Dict[str, int]) -> None:
    for jn in ALOHA_GRIPPER_JOINT_NAMES:
        jid = jmap.get(jn, None)
        if jid is None:
            continue
        p.setJointMotorControl2(robot_id, jid, p.POSITION_CONTROL, targetPosition=0.017, force=40)

def aloha_close_gripper(robot_id: int, jmap: Dict[str, int]) -> None:
    for jn in ALOHA_GRIPPER_JOINT_NAMES:
        jid = jmap.get(jn, None)
        if jid is None:
            continue
        p.setJointMotorControl2(robot_id, jid, p.POSITION_CONTROL, targetPosition=0.0, force=60)
...
# ============================================================
# ALOHA URDF sanitization (robot-only safety fix)
# ============================================================

def _safe_mesh_exists(urdf_dir: str, filename: str) -> bool:
    if not filename:
        return False
    path = filename
...
def _to_abs_mesh_path(urdf_dir: str, filename: str) -> str:
    if not filename:
        return filename
...
def prepare_aloha_urdf_for_pybullet(urdf_path: str) -> str:
    """Return a PyBullet-safe URDF path for ALOHA.

    Fixes robot-side URDF issues only:
    - removes the duplicate legacy joint 'finger_l';
    - preserves/loading meshes correctly by converting mesh paths to absolute
      paths when a sanitized temporary URDF is written;
    - if meshes are missing, strips visual/collision nodes but keeps the
      kinematic chain intact.
    """
    ...
    for mesh in root.findall(".//mesh"):
        fname = mesh.attrib.get("filename", "").strip()
        if fname and not _safe_mesh_exists(urdf_dir, fname):
            missing_mesh = True
        elif fname:
            mesh.set("filename", _to_abs_mesh_path(urdf_dir, fname))

    # Remove duplicate legacy joint attached to the same child link.
    for joint in list(root.findall("joint")):
        if joint.attrib.get("name") == "finger_l":
            root.remove(joint)
    ...
    # Keep the sanitized URDF in a temp file; mesh filenames are absolute now.
    fd, tmp_path = tempfile.mkstemp(prefix="aloha_pybullet_", suffix=".urdf")
    os.close(fd)
...
def setup_pybullet_world(
    ...
    *,
    ...
    urdf_path = os.path.abspath(os.path.expanduser(aloha_urdf))
    if not os.path.isfile(urdf_path):
        raise FileNotFoundError(f"ALOHA LC v1 URDF not found: {urdf_path}")

    urdf_dir = os.path.dirname(urdf_path)
    p.setAdditionalSearchPath(urdf_dir)

    safe_urdf_path = prepare_aloha_urdf_for_pybullet(urdf_path)
    safe_urdf_dir = os.path.dirname(safe_urdf_path)
    p.setAdditionalSearchPath(safe_urdf_dir)

    # Place base close enough to the first object
    if ycb_positions:
        ox, oy, _ = ycb_positions[0]
        robot_base_xyz = [float(ox - 0.28), float(oy - 0.30), 0.0]
    else:
        robot_base_xyz = [0.18, -0.30, 0.0]

    log(f"Loading ALOHA LC v1 URDF: {urdf_path}")
    log(f"URDF assets dir: {urdf_dir}")
    if safe_urdf_path != urdf_path:
        log(f"Using sanitized ALOHA URDF for PyBullet: {safe_urdf_path}")

    base_yaw = 1.5708  # 90 grados en radianes
    ...
    jmap = get_aloha_joint_map(robot_id)
    missing = [jn for jn in ALOHA_ARM_JOINT_NAMES + ALOHA_GRIPPER_JOINT_NAMES if jn not in jmap]
    if missing:
